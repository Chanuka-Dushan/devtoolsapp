 ...
 *
 * Uses the Web Crypto API (SubtleCrypto) for SHA algorithms.
 * MD5 is implemented in pure JavaScript (not cryptographically secure — for legacy use only).
 *
 * All computation happens client-side. Input is never sent to a server.
 */
 ...
 * Uses SubtleCrypto for SHA algorithms.
 */
...
export async function generateAllHashes(
  ...
  const results = await Promise.all(
    algorithms.map((alg) => generateHash(input, alg))
  );
  return Object.fromEntries(
    algorithms.map((alg, i) => [alg, results[i]])
  ) as Record<HashAlgorithm, HashResult>;
}

// ─── MD5 Implementation (pure JS) ─────────────────────────────────────────
// Note: MD5 is NOT cryptographically secure. Use SHA-256 or better for security purposes.
